...
sys.path.insert(0,'eval_files/transe_eval_files')
import np_eval
import tf_eval
...
class TransE(KBC_Class):
    ...
    def __init__(self, dataset, swap, dim, margin, l1_flag, device, memory, learning_rate, max_epoch, batch_size, test_size, result_log_cycle, eval_with_np = True, shuffle_data=True, check_collision=True, normalize_ent=True):
    
        KBC_Class. __init__(self, dataset, swap, 'TransE', dim, margin, device, memory, learning_rate, max_epoch, batch_size, test_size, result_log_cycle, eval_with_np = True, shuffle_data=True, check_collision=True, normalize_ent=True)
    ...
    def np_score(self, h,l,t):
        if self.l1_flag:
            return -1 * np.linalg.norm((h+l-t), ord=1, axis=1)
        else: 
            return -1 * np.linalg.norm(h+l-t, axis=1)
	...
	# in case model parameters have to be adapted before used in evaluation script
	...
	#model = self.adapt_params_for_eval(model)
	if eval_mode: 
		top_triples = eval.run_evaluation(triples_set, test_matrix, model, score_func=self.tf_score, test_size=self.test_size, eval_mode=True, filtered=filtered, verbose=True)
		return top_triples
	record = eval.run_evaluation(triples_set, test_matrix, model, score_func=self.tf_score, test_size=self.test_size, filtered=filtered)
	return record
