...
class Bilinear_Decomp(KBC_Class):
    ...
    # overwriting getPATHS method for this model to include the parameter a (hidden dimension) for directory where model is saved
    def getPATHS(self):
        # set working directory to current directory and based on this set all required PATHS
        os.chdir(os.getcwd())
        PATH = self.DATA_PATH + 'Trained Models/'+ self.model_name+'/' + self.dataset + '/dim = '+str(self.dim) +'/a = '+ str(self.dim_hidden) + '/'
