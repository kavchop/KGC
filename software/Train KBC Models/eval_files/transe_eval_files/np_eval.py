...
def run_evaluation(triples_set, test_matrix, model, score_func, test_size=None, eval_mode=False, filtered=False, verbose=False): 
	...
	#label = np.repeat(np.reshape(rel_array_map[test_matrix[i,1]], (1,dim)), n, axis=0)
        #correct_score = score_func(ent_array_map[test_matrix[i,0]], rel_array_map[test_matrix[i,1]], ent_array_map[test_matrix[i,2]])
	correct_score = '-'
	...
	scores = score_func(head, label, tail)
	...
	tail = ent_array_map[triple_matrix_t[:,2]] 
        if filtered: 	
		label = rel_array_map[triple_matrix_t[:,1]] 
