...
def run_evaluation(triples_set, total_test_matrix, ent_array_map, rel_array_map, 
    ...
    # determine number of iterations based on test_size and batch_size
    num_batches = test_size/batch_size
    test_size = num_batches * batch_size  #adapted test_size divadable by 3
    ...
    sub_matrix_list = np.split(total_test_matrix, num_batches)
	...
	init_op = tf.global_variables_initializer()
		...
		for i in range(len(test_matrix)): 
		        ...
		        a_h = a_h_map[i]
		        a_t = a_t_map[i]
                        correct_score = -1 * calc_dissimilarity(ent_array_map[test_matrix[i,0]]+rel_array_map[test_matrix[i,1]]-ent_array_map[test_matrix[i,2]], l1_flag)
        ...
        final_result = list(np.array(result_sum/num_batches, dtype=np.int32))
        if eval_mode: 
                print "Final Result: "
        	print final_result
        return final_result
