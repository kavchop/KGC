...
def run_training(model_name): 
    ...
    if os.path.isfile(MODEL_PATH):
        ...
        W_param = model[0]
        A_param = np.array(model[1], dtype=np.float32)
        B_param = np.array(model[2], dtype=np.float32)

        # if 'evaluate' tag was passed when running the script, only run evaluation on test-set, save top triples and terminate
    if eval_mode:    
        W_eval_param, Mr_eval_param = input.adapt_params_for_eval(W_param, A_param, B_param)
        ...
        W_param, A_param, B_param = input.init_params(n, m, n_red, a)
        model = [W_param, A_param, B_param]
    ...
    with g.as_default(), g.device('/'+device+':0'), tf.Session() as sess: 
        ...
        print "\n****************Training of Decomposed Bilinear Model starts!****************\n"
        print "Number of Triples in Training data: {}".format(len(train_matrix))
        print "Latent Embedding Dimension n_red: {}".format(n_red)
        print "Latent Embedding Dimension a of Relation Matrices: {}".format(a)
