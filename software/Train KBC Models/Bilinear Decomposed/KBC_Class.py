...
class KBC_Class:
    def __init__(self, dataset, swap, model_name, dim, margin, device, learning_rate, max_epoch, batch_size, test_size, result_log_cycle, shuffle_data=True, check_collision=True, normalize_ent=True):
    ...
    def getPATHS(self):
        #set working directory to current directory and based on this set all required PATHS
        os.chdir(os.getcwd())
        PATH = DATA_PATH + 'Trained Models/'+ self.model_name+'/' + self.dataset + '/dim = '+str(self.dim) +'/'
        if not os.path.exists(PATH):
            os.makedirs(PATH)

        PLOT_PATH = DATA_PATH + 'Model Validation Results for Plotting/' + self.dataset + '/dim = '+str(self.dim) +'/'
    ...
    def load_data(self): 
        DATA_SET_PATH = DATA_PATH + 'Triple Store/' + self.dataset + '/'
        ...
        if not os.path.isfile(DATA_SET_PATH +'train.txt') or not os.path.isfile(DATA_SET_PATH +'test.txt') or not os.path.isfile(DATA_SET_PATH +'valid.txt'): 
            zip_ref = zipfile.ZipFile(DATA_SET_PATH +'data.zip', 'r')
