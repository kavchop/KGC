...
def run_evaluation(triples_set, test_matrix, ent_array_map, rel_array_map, 
    ...
    for i in range(len(test_matrix)):
        ...
        if test_matrix[i,1] in thresholds:
	    if thresholds[test_matrix[i,1]] < correct_score:
	    	thresholds[test_matrix[i,1]] = correct_score 
        else:
            thresholds[test_matrix[i,1]] = correct_score
    ...
    if eval_mode: 
        ...
        print "Number of relations covered for threshols: {}".format(len(thresholds))
        pickle_object('classification thresholds', 'w', thresholds)
