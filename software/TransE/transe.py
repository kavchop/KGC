...
def run_training(): 
	...
	if eval_mode:	 
        ...
        input.save_model_meta(model_name, dim, learning_rate, normalize_ent, check_collision)
        ent_array_map, rel_array_map = input.create_embed_maps_from_int(n,m,dim)
        transE_model = [ent_array_map,rel_array_map]
        pickle_object(INITIAL_MODEL, 'w', transE_model)
