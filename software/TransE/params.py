...
result_log_cycle = 2        #run validation and log results after every x epochs
embedding_log_cycle = 2     #save embedding to disk after every x epochs
valid_size = 50           #to speed up validation validate on a randomly drawn set of x from validation set
valid_verbose = False       #display scores for each test triple during validation 
train_verbose = True       #display loss after each gradient step during training
normalize_ent = False  	    #normalization of entity vectors after each gradient step        
