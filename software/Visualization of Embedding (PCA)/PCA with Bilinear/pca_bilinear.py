...
INITIAL_MODEL = 'bilinear_initial_model'
MODEL_PATH = 'bilinear_model'
model_name = 'Bilinear'
...
def main(arg=None):
    ...
    learned_red_h = PCA(np.transpose(initial_data[0]))
    learned_red_l = PCA(np.transpose(initial_data[1]))
    learned_red_t = PCA(np.transpose(initial_data[2]))
