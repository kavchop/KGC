...
def plotData_scatter_all(n, x, y, title_prefix, num_plot=None, sub_sample=None, rel_sample=None, obj_sample=None):
    ...
    plt.xlim([-1,1])
    plt.ylim([-1,1])
    ax.set_title(title_prefix +" TransE Embedding: 2D Scatter View of Relations")
    s = 100
...
def main(arg=None):
    ...
    if rel_type == 'random': 
         plotData_scatter(n, initial_red_x, initial_red_y, 'Initial', num_plot, sub_sample=subject_sample, rel_sample=relation_sample, obj_sample=object_sample)
         plotData_scatter(n, learned_red_x,learned_red_y, 'Learned', num_plot, sub_sample=subject_sample, rel_sample=relation_sample, obj_sample=object_sample)

	 plotData_scatter_all(n, initial_red_x, initial_red_y, 'Initial', num_plot, sub_sample=subject_sample, rel_sample=relation_sample, obj_sample=object_sample)
    plotData_scatter_all(n, learned_red_x,learned_red_y, 'Learned', num_plot, sub_sample=subject_sample, rel_sample=relation_sample, obj_sample=object_sample)

    

    if rel_type == 'one to many' or rel_type == 'many to one':
    	plotData_rel_type(n, initial_red_x, initial_red_y, 'Initial', num_plot, sub_sample=subject_sample, rel_sample=relation_sample, obj_sample=object_sample)
    	plotData_rel_type(n, learned_red_x,learned_red_y, 'Learned', num_plot, sub_sample=subject_sample, rel_sample=relation_sample, obj_sample=object_sample)
