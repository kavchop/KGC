...
def create_triple_array_batches(matrix_batch, ent_array_map, rel_array_map, ent_p_array_map, rel_p_array_map, corrupt=False):
    ...
    hp_batch =  np.asarray([ent_p_array_map[matrix_batch[i,0]] for i in range(len(matrix_batch))])
    tp_batch =  np.asarray([ent_p_array_map[matrix_batch[i,2]] for i in range(len(matrix_batch))])
    if corrupt: 
        return h_batch, t_batch, hp_batch, tp_batch
    l_batch =  np.asarray([rel_array_map[matrix_batch[i,1]] for i in range(len(matrix_batch))])
    lp_batch =  np.asarray([rel_p_array_map[matrix_batch[i,1]] for i in range(len(matrix_batch))])
    return h_batch, l_batch, t_batch, hp_batch, lp_batch, tp_batch
...
def create_embed_maps_from_int(n,m, dim_ent, dim_rel):
    ent_array_map = np.random.uniform(-6/sqrt(dim_ent),6/sqrt(dim_ent), (n, dim_ent))
    rel_array_map = np.random.uniform(-6/sqrt(dim_rel),6/sqrt(dim_rel), (m, dim_rel))

    ent_p_array_map = np.random.uniform(-6/sqrt(dim_ent),6/sqrt(dim_ent), (n, dim_ent))
    rel_p_array_map = np.random.uniform(-6/sqrt(dim_rel),6/sqrt(dim_rel), (m, dim_rel))
    
    ent_array_map = normalize_embedding(ent_array_map)
    rel_array_map = normalize_embedding(rel_array_map)
    ent_p_array_map = normalize_embedding(ent_p_array_map)
    rel_p_array_map = normalize_embedding(rel_p_array_map)
    return ent_array_map, rel_array_map, ent_p_array_map, rel_p_array_map


#method writes model configurations to disk 
def save_model_meta(model_name, dim_ent, dim_rel, learning_rate, normalize_ent, check_collision):
    text_file = open("models/"+model_name+"_model_meta.txt", "w")
    text_file.write("\nmodel: {}\n\n".format(model_name))

    text_file.write("created on: {}\n".format(datetime.now().strftime('%d-%m-%Y %H:%M:%S')))
    text_file.write("entity dimension:  {}\n".format(dim_ent))
    text_file.write("relation dimension:  {}\n".format(dim_rel))
