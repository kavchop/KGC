...
def get_data_for_roc(pos_matrix, neg_matrix, ent_array_map, rel_array_map):
		...
		#if abs(f1-f2)<1e-6
		pos_score = score_func(h_batch[i], l_batch[i], t_batch[i], l1_flag)
		#print pos_score
		neg_score = score_func(h_1_batch[i], l_batch[i], t_1_batch[i], l1_flag)
...
def load_model(TransE_Model):
    if os.path.isfile(DATA_PATH +'URI_to_int'): 
        URI_to_int = TransE_Model.pickle_object(DATA_PATH +'URI_to_int', 'r')
        ent_URI_to_int = URI_to_int[0]
        rel_URI_to_int = URI_to_int[1]
        rel_list = rel_URI_to_int.keys() 
	if os.path.isfile(MODEL_PATH):
		model = TransE_Model.pickle_object(MODEL_PATH, 'r')
		#ent_array_map = model[0]
		#rel_array_map = model[1]
                init_model = TransE_Model.pickle_object(INITIAL_MODEL, 'r')
...
def find_thresholds(triples_matrix, ent_array_map, rel_array_map, l1_flag):
    thresholds = {}
	...
	score = score_func(head, label, tail, l1_flag) 
...
def score_func(h, l, t, l1_flag=True):
    if l1_flag:
        return -1 * np.linalg.norm((h, l, t), ord=1)
    else: 
        return -1 * np.linalg.norm(h, l, t)
...
def main(arg=None):
    ...
    model, init_model, rel_list = load_model(TransE_Model) 
    ent_array_map = model[0]
    rel_array_map = model[1]
    ...
    thresholds = find_thresholds(triples_matrix, ent_array_map, rel_array_map, l1_flag)
    ...
    for i in range(len(pos_matrix)):
        if score_func(h_batch[i], l_batch[i], t_batch[i], l1_flag) < thresholds[pos_matrix[i,1]]:
            fn +=1
        else: 
            tp +=1
            
        if score_func(h_1_batch[i], l_batch[i], t_1_batch[i], l1_flag) < thresholds[pos_matrix[i,1]]:
            tn +=1
        else: 
            fp +=1
