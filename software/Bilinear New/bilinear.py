...
#score: x0 * Mr * x1
def Layer_2(x_0_pos, x_1_pos, x_0_neg, x_1_neg, A, B): 
   
    layer_2_pos = tf.matmul((tf.matmul(x_0_pos, tf.matmul(A, tf.transpose(B)))), tf.transpose(x_1_pos))   #[none, none] = [none, n_red] * [n_red, n_red] * [n_red, none]
    layer_2_neg = tf.matmul((tf.matmul(x_0_neg, tf.matmul(A, tf.transpose(B)))), tf.transpose(x_1_neg))
...
def run_training(model_name): 
    ...
    print "\nBilinear model is {} with 2 non-diagonal relation matrices.\n".format(mode)
    ...
    if os.path.isfile(MODEL_PATH):
        ...
        A_param = bilinear_model[1]
	B_param = bilinear_model[2]
	...
	if eval_mode:	 
		...
		eval.run_evaluation(triples_set, valid_matrix, entity_embed, Mr_param, eval_mode=True, verbose=True, test_size=valid_size) 
		return
    else: 
        if eval_mode: 
	...
	with open('models/transe_model_meta.txt', "a") as f:
        ...
        Mr_param = np.array([np.dot(A_param[i], np.transpose(B_param[i])) for i in range(m)])
        entity_embed = input.learned_ent_embed(ent_array_map, W_param)
        record = eval.run_evaluation(triples_set, valid_matrix, entity_embed, Mr_param, test_size=valid_size)  
    ...
    A = tf.Variable(tf.zeros([n_red, a]))
    A_ph = tf.placeholder(tf.float32, shape=(n_red, a))
    A_op = tf.assign(A, A_ph)
    B = tf.Variable(tf.zeros([n_red, a]))
    B_ph = tf.placeholder(tf.float32, shape=(n_red, a))
    B_op = tf.assign(B, B_ph)
    ...
    pos_score, neg_score = Layer_2(x_1, x_2, x_3, x_4, A, B)
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_epoch):
            ...
            for j in range(m):
                ...
                sess.run(A_op, feed_dict = {A_ph: A_param[j]}) 
                sess.run(B_op, feed_dict = {B_ph: B_param[j]}) 
		...
		if normalize_ent: 
            ...
            if global_epoch == 1 or global_epoch%result_log_cycle == 0:  #may change j to num-epoch 
                Mr_param = np.array([np.dot(A_param[i], np.transpose(B_param[i])) for i in range(m)])
                entity_embed = input.learned_ent_embed(ent_array_map, W_param)
                record = eval.run_evaluation(triples_set, valid_matrix, entity_embed, Mr_param, test_size=valid_size, verbose=valid_verbose) 
                new_record = np.reshape(np.asarray([global_epoch]+record), (1,5))
